...
function _setupListeners(stomp) {
    ...
    socket.on('data', function(chunk) {
        ...
        // To handle keepalive message from AMQ, this will just ignore keepalive and clear buffer
        if (frames.length == 1) {
            frames = buffer.split('\n');
            if (frames.length == 2 && frames[1] == '' ){
                log.debug("frames[0]: "+frames[0]+"frames[1]: "+frames[1]);
                buffer = '';
            }
            return;
        }
